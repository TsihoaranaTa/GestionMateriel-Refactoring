<?php
	...
	$nb_materiels = Materiel::nb();	//on sauvegarde dans une variable pour ne pas consulter la base une deuxiéme fois
	if(!Utilisateur::utilisateurConnecte()):	//si l'utilisateur n'est pas connecté, on le renvoie vers l'interface de connexion
		Application::redir('../login/');
	endif;
	if(Materiel::pasDelements()):	//test pour vérifier qu'il y a des materiels à afficher
		Application::alert("aucun matériel trouvée!");
...
<html>
<head>
  <title><?php echo "afficher_materiel ($nb_materiels)"; ?></title>
...
<body>
  ...
  <div id="main">
    ...
    <div id="site_content">
	<div id="content">
		<h2>Gestion de Matériel</h2>
		<br/>
		<?php
			$table = Materiel::getTableName();	//nom de la table à consulter
			...
			if(Sort::parametresExistent()):
				...
				if(Sort::parametresValides()):
					if(preg_match("~^-?[0-9]+$~i", $_GET['n'])):
						...
						$nb_lignes = $nb_materiels;	//nombre totale de lignes
				...
				if(isset($_GET['n'])):
					if(preg_match("~^-?[0-9]+$~i", $_GET['n'])):
						$n = (int) $_GET['n'];				//paramétre passé à la page
						$nb_lignes = Materiel::nb();	//nombre totale de lignes
						$nlpp = Application::Lpp();							//nombre de lignes par page
						$derniere_page = ceil($nb_lignes / $nlpp);
						if(!$nb_lignes):	//aucun résultat à afficher
							$error = true;
			...
			if(!$error):
				...
				<table class="table container-md table-hover justify-content-center" >
					...
					<tr>
						...
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_direction&ordre=<?php echo Sort::$orderLog['id_direction']; ?>&n=<?php echo $_GET['n']; ?>">Nom direction <?php Sort::printArrow('id_direction'); ?></a></th>

						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_departement&ordre=<?php echo Sort::$orderLog['id_departement']; ?>&n=<?php echo $_GET['n']; ?>">Département rattaché <?php Sort::printArrow('id_departement'); ?></a></th>

						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_service&ordre=<?php echo Sort::$orderLog['id_service']; ?>&n=<?php echo $_GET['n']; ?>">Nom service <?php Sort::printArrow('id_service'); ?></a></th>
						...
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=type&ordre=<?php echo Sort::$orderLog['type']; ?>&n=<?php echo $_GET['n']; ?>">Type de matériel <?php Sort::printArrow('type'); ?></a></th>
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=reference&ordre=<?php echo Sort::$orderLog['reference']; ?>&n=<?php echo $_GET['n']; ?>">Références <?php Sort::printArrow('reference'); ?></a></th>
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=date&ordre=<?php echo Sort::$orderLog['date']; ?>&n=<?php echo $_GET['n']; ?>">date réception <?php Sort::printArrow('date'); ?></a></th>
						...
						foreach($res as $record):
							?>
								<tr>
									...
									<td><?php echo Direction::get($record['id_direction'])->getNom(); ?></td>
									<td><?php echo Departement::get($record['id_departement'])->getNom(); ?></td>
									<td><?php echo Service::get($record['id_service'])->getNom(); ?></td>
									<td><?php echo Personnel::get($record['id_personnel'])->getNom(); ?></td>
									
									<td><?php echo $record['type']; ?></td>
									<td><?php echo $record['reference']; ?></td>
									<td><?php echo $record['date']; ?></td>
									<?php if(Utilisateur::getUtilisateurConnecte()->estAdmin()): ?>
										<td><a href="modifier_materiel.php?id=<?php echo $record['id']; ?>"><i class="fas fa-pen" style="color: #3F729B; font-size: 25px;"></i></a></td>
										<td><a href="supprimer_materiel.php?id=<?php echo $record['id']; ?>" onclick="return(confirm('Supprimer?'))"><i class="fas fa-trash-alt" style="color:#ff4444; font-size: 25px;"></i></a></td>
									<?php endif; ?>
								</tr>
							<?php
						endforeach;
		...
		<br /><br /><a href="../include/telecharger_liste.php?table=materiels" target="_blank"><img src="../include/img/download.png" style="height:25px;width:25px;" title="télécharger"/></a>
	</div>
    </div>
</div>
</div>
