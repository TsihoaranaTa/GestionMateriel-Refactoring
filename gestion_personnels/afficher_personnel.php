<?php
	...
	$nb_personnels = Personnel::nb();	//on sauvegarde dans une variable pour ne pas consulter la base une deuxiéme fois
	if(!Utilisateur::utilisateurConnecte()):	//si l'utilisateur n'est pas connecté, on le renvoie vers l'interface de connexion
		Application::redir('../login/');
	endif;
	if(Personnel::pasDelements()):	//test pour vérifier qu'il y a des gadgets à afficher
		Application::alert("aucun personnel trouvée!");
...
<html>
<head>
  <title><?php echo "afficher_personnel ($nb_personnels)"; ?></title>
...
<body>
  ...
  <div id="main">
    ...
    <div id="site_content">
	<div id="content">
		<h2>Gestion de personnel</h2>
		<br/>
		<?php
			$table = Personnel::getTableName();	//nom de la table à consulter
			...
			if(Sort::parametresExistent()):
				...
				if(Sort::parametresValides()):
					if(preg_match("~^-?[0-9]+$~i", $_GET['n'])):
						...
						$nb_lignes = $nb_personnels;	//nombre totale de lignes
				...
				if(isset($_GET['n'])):
					if(preg_match("~^-?[0-9]+$~i", $_GET['n'])):
						$n = (int) $_GET['n'];				//paramétre passé à la page
						$nb_lignes = Personnel::nb();	//nombre totale de lignes
						$nlpp = Application::Lpp();							//nombre de lignes par page
						$derniere_page = ceil($nb_lignes / $nlpp);
						if(!$nb_lignes):	//aucun résultat à afficher
							$error = true;
			...
			if(!$error):
				...
				<table class="table container-md table-hover justify-content-center" >
					...
					<tr>
						...
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_departement&ordre=<?php echo Sort::$orderLog['id_departement']; ?>&n=<?php echo $_GET['n']; ?>">nom_departement <?php Sort::printArrow('id_departement'); ?></a></th>

						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_service&ordre=<?php echo Sort::$orderLog['id_service']; ?>&n=<?php echo $_GET['n']; ?>">nom_service <?php Sort::printArrow('id_service'); ?></a></th>
						...
						foreach($res as $record):
							?>
								<tr>
									...
									<?php if(Utilisateur::getUtilisateurConnecte()->estAdmin()): ?>
										<td><a href="modifier_personnel.php?id=<?php echo $record['id']; ?>"><i class="fas fa-pen" style="color: #3F729B; font-size: 25px;"></i></a></td>
										<td><a href="supprimer_personnel.php?id=<?php echo $record['id']; ?>" onclick="return(confirm('Supprimer?'))"><i class="fas fa-trash-alt" style="color:#ff4444; font-size: 25px;"></i></a></td>
									<?php endif; ?>
								</tr>
							<?php
						endforeach;
