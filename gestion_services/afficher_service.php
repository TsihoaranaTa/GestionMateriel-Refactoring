<?php
	...
	$nb_services = Service::nb();	//on sauvegarde dans une variable pour ne pas consulter la base une deuxiéme fois
	if(!Utilisateur::utilisateurConnecte()):	//si l'utilisateur n'est pas connecté, on le renvoie vers l'interface de connexion
		Application::redir('../login/');
	endif;
	if(Service::pasDelements()):	//test pour vérifier qu'il y a des services à afficher
		Application::alert("aucun service trouvée!");
...
<html>
<head>
  <title><?php echo "afficher_service ($nb_services)"; ?></title>
...
<body>
  ...
  <div id="main">
    ...
    <div id="site_content">
	<div id="content">
		<h2>Gestion de service</h2>
		<br/>
		<?php
			$table = Service::getTableName();	//nom de la table à consulter
			...
			if(Sort::parametresExistent()):
				...
				if(isset($_GET['n'])):
					if(preg_match("~^-?[0-9]+$~i", $_GET['n'])):
						$n = (int) $_GET['n'];				//paramétre passé à la page
						$nb_lignes = Service::nb();	//nombre totale de lignes
						$nlpp = Application::Lpp();							//nombre de lignes par page
						$derniere_page = ceil($nb_lignes / $nlpp);
						if(!$nb_lignes):	//aucun résultat à afficher
							$error = true;
			...
			if(!$error):
				...
				<table class="table container-md table-hover justify-content-center">
					...
					<tr>
						...
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_direction&ordre=<?php echo Sort::$orderLog['id_direction']; ?>&n=<?php echo $_GET['n']; ?>">id_direction <?php Sort::printArrow('id_direction'); ?></a></th>
						...
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_departement&ordre=<?php echo Sort::$orderLog['id_departement']; ?>&n=<?php echo $_GET['n']; ?>">id_departement <?php Sort::printArrow('id_departement'); ?></a></th>
						<th><a href="<?php echo $_SERVER['PHP_SELF']; ?>?col=id_departement&ordre=<?php echo Sort::$orderLog['id_departement']; ?>&n=<?php echo $_GET['n']; ?>">nom_dep <?php Sort::printArrow('id_departement'); ?></a></th>
						...
						foreach($res as $record):
							?>
								<tr>
									...
									<td><?php echo $record['id_departement']; ?></td>
									<td><?php echo Departement::get($record['id_departement'])->getNom(); ?></td>
									<td><?php echo $record['nom']; ?></td>
									
									<?php if(Utilisateur::getUtilisateurConnecte()->estAdmin()): ?>
										<td><a href="modifier_service.php?id=<?php echo $record['id']; ?>"><i class="fas fa-pen" style="color: #3F729B; font-size: 25px;"></i></a></td>
										<td><a href="supprimer_service.php?id=<?php echo $record['id']; ?>" onclick="return(confirm('Supprimer?'))"><i class="fas fa-trash-alt" style="color:#ff4444; font-size: 25px;"></i></a></td>
									<?php endif; ?>
								</tr>
							<?php
						endforeach;
