<?php
	...
	if(Produit::pasDelements()):	//test pour vérifier qu'il y a des produits à modifier
		Application::alert("aucune sortie trouvé!");
	...
	$sortie = Sortie::get($cleId);
	$quantite = (isset($_SESSION['SortieQuantite']) && !empty($_SESSION['SortieQuantite']))? $_SESSION['SortieQuantite']: $sortie->getQuantite();
	$bon = (isset($_SESSION['SortieBon']) && !empty($_SESSION['SortieBon']))? $_SESSION['SortieBon']: $sortie->getBon();
	$dateSortie = (isset($_SESSION['SortieDateSortie']) && !empty($_SESSION['SortieDateSortie']))? $_SESSION['SortieDateSortie']: $dateSortie->getDateSortie();
  ...
  <form method="post" action="mod.php?id=<?php echo $_GET['id']; ?>" class="login ">
  		<fieldset>
			...
			<div class="form-group row justify-content-center">
				<label for="inputEmail3MD " class="col-sm-2 col-form-label">Bon</label>
			<div class="col-sm-3 ">
		      <div class="md-form mt-0">
		        <input type="text" name="bon" class="form-control" id="inputEmail3MD" required value="<?php echo $bon; ?>"/>
		      </div>
		    </div>
			...
			<div>
				...
				<input type="number" min="0" name="quantite" placeholder="Quantité de sortie" required value="<?php $quantite; ?>"/>
			</div>
	
			<span><?php echo $message; $_SESSION['message'] = ''; ?></span>
  		</fieldset>
		<button class="btn btn-default" type="submit" name="modifier_sortie" class="btn btn-default">Modifier</button>
  </form>
  </div>
            <div class="col-4 col-sm-2">
